...
async function sendToSheets(data: FormData) {
  try {
    ...
    // Img tag ignora CORS completamente
    ...
    return (
      <section id="formulario" className="section-cream section-padding">
        <div className="max-w-lg mx-auto text-center rounded-lg border border-[hsl(var(--border))] bg-white p-12">
          ...
          <h2 className="font-display text-2xl font-semibold mb-3">Inscrição recebida!</h2>
          <p className="text-[hsl(var(--dark)/0.7)] leading-relaxed mb-6">
            ...
            entrará em contato pelo WhatsApp em breve.
          </p>
          <p className="text-sm text-[hsl(var(--dark)/0.5)] mb-5">
            Redirecionando para o WhatsApp em instantes…
          </p>
          <a href={WA_URL} target="_blank" rel="noopener noreferrer" className="btn-gold inline-flex">
    ...
    <section id="formulario" className="section-cream section-padding">
      <div className="max-w-xl mx-auto">
        <div className="text-center mb-10">
          ...
          <h2 className="font-display text-3xl md:text-4xl mt-3 font-semibold">Reserve Sua Vaga</h2>
        ...
        <div className="flex items-center gap-2 mb-8" aria-hidden="true">
          {STEPS.map((label, i) => (
            <div key={label} className="flex-1">
              <div className={`h-1.5 rounded-full transition-colors ${i <= step ? "bg-[hsl(var(--gold-dark))]" : "bg-[hsl(var(--cream-dark))]"}`} />
              <p className={`mt-2 text-[11px] text-center font-medium ${i === step ? "text-[hsl(var(--gold-dark))]" : "text-[hsl(var(--dark)/0.4)]"}`}>
                {label}
              </p>
            </div>
          ))}
        ...
        <div className="rounded-lg border border-[hsl(var(--border))] bg-white p-7 md:p-9">
          {step === 0 && (
            <fieldset className="space-y-4">
              <legend className="font-display text-lg font-semibold mb-2">Seus dados de contato</legend>
          ...
          {step === 1 && (
            <fieldset className="space-y-4">
              <legend className="font-display text-lg font-semibold mb-2">Sua formação</legend>
              <Field label="Especialidade/Formação" value={data.especialidade} onChange={(v) => update("especialidade", v)} />
              <Select label="Nível de Formação" value={data.nivelFormacao} onChange={(v) => update("nivelFormacao", v)} options={["Fellow", "Residente", "Preceptor"]} />
              <Select label="Nível de Conhecimento em Artroscopia de ATM" value={data.nivelConhecimento} onChange={(v) => update("nivelConhecimento", v)} options={["Iniciante", "Intermediário", "Avançado"]} />
            </fieldset>
          )}

          {step === 2 && (
            <fieldset className="space-y-5">
              <legend className="font-display text-lg font-semibold mb-2">Quase lá</legend>
              <Field label="Como conheceu este programa? (opcional)" value={data.comoConheceu} onChange={(v) => update("comoConheceu", v)} />
              ...
              <label className="flex items-start gap-3 text-sm text-[hsl(var(--dark)/0.75)]">
                <input type="checkbox" checked={data.consentimento} onChange={(e) => update("consentimento", e.target.checked)} className="mt-1 h-4 w-4 accent-[hsl(var(--gold-dark))]" />
                <span>
                  Aceito receber atualizações sobre o programa por e-mail ou WhatsApp. Seus dados são tratados conforme nossa{" "}
                  <a href="/politica-de-privacidade" className="underline hover:text-[hsl(var(--gold-dark))]">Política de Privacidade</a>.
                </span>
              </label>
            </fieldset>
          )}

          {error && <p role="alert" className="mt-4 text-sm text-[hsl(var(--burgundy))]">{error}</p>}

          <div className="mt-8 flex items-center justify-between gap-4">
            <button type="button" onClick={back} disabled={step === 0} className="inline-flex items-center gap-1.5 text-sm font-medium text-[hsl(var(--dark)/0.6)] disabled:opacity-0">
